...
@with_session
def list_kbs_from_db_all(session,min_file_count: int = -1):
    kbs = session.query(KnowledgeBaseModel.kb_name).filter(KnowledgeBaseModel.file_count > min_file_count).all()
    kbs_info = session.query(KnowledgeBaseModel.kb_info).filter(KnowledgeBaseModel.file_count > min_file_count).all()
    kbs = [kb[0] for kb in kbs]
    kbs_info = [kb[0] for kb in kbs_info ]
    return [kbs, kbs_info]
